...
The KITTI-360 dataset~\cite{liao2022kitti360} is a large-scale outdoor benchmark for autonomous driving research, extending the original KITTI dataset~\cite{geiger2012are} with substantially richer semantic and geometric annotations. The data were collected in suburban areas of Karlsruhe, Germany, using a vehicle equipped with two high-resolution stereo cameras, a Velodyne HDL-64E 3D laser scanner, and an OXTS RT3003 GPS/IMU localisation unit. The full dataset comprises approximately 73,000 front-facing camera frames distributed across 11 driving sequences, accompanied by dense 2D semantic and instance segmentation maps, 3D bounding boxes, and accumulated LiDAR point clouds.

This work uses the front-facing camera images together with their corresponding 2D semantic segmentation annotations. The dataset is partitioned into training and validation splits along sequence boundaries, ensuring that no frames from the same drive appear in both sets. This non-overlapping split prevents temporal data leakage and guarantees that evaluation is performed on scenes the model has not encountered during training.

\subsection{Semantic Annotations}
\label{subsec:semantic_annotations}

The 2D semantic annotations provided with KITTI-360 follow the Cityscapes label taxonomy~\cite{cordts2016cityscapes}, encompassing 19 training categories that cover the principal scene elements of urban and suburban driving environments. Raw dataset annotation identifiers are remapped to contiguous training indices (trainIDs 0--18) according to the official label mapping. Table~\ref{tab:kitti360_classes} lists all 19 semantic categories together with their assigned training indices.
...
The class distribution across the dataset is strongly imbalanced: large-area categories such as road, building, and vegetation dominate the pixel counts, whereas classes such as traffic lights, motorcycles, and bicycles occupy relatively few pixels. This imbalance has direct consequences for training and evaluation. During training, the Semantic VAE loss formulation must account for class frequency to avoid the model collapsing to predictions that favour the majority classes. During evaluation, standard pixel accuracy is inadequate as a sole metric for the same reason; frequency-weighted and class-balanced measures are therefore reported in parallel, as described in Section~\ref{subsec:miou}.

\subsection{Data Preparation and Clip Construction}
\label{subsec:data_preparation}

Prior to training, all images are resized from the native camera resolution of $376 \times 1408$ pixels to $192 \times 704$ pixels. Bilinear interpolation is applied to RGB frames to preserve smooth gradients, while nearest-neighbour interpolation is used for semantic label maps to prevent the introduction of spurious class labels at region boundaries.

Consecutive annotated frames are grouped into non-overlapping temporal clips for model training. Clips of $T = 25$ frames are used for both Stage~1 and Stage~2 diffusion training, consistent with the Stable Video Diffusion XT (SVD-XT)~\cite{blattmann2023stable} backbone, which operates natively on 25-frame sequences. A shorter clip length of $T = 4$ frames is used for Semantic VAE training, which is appropriate given the compact size of the trainable adapter modules and the fact that the VAE encodes each frame independently. RGB pixel values are normalised to the range $[-1, 1]$. Semantic labels are retained as integer class indices throughout the data pipeline and are converted to one-hot representations only at the Semantic VAE input stage, as described in Section~\ref{subsec:semantic_native_vae}.

The training split yields approximately 49,000 annotated frame pairs. For Semantic VAE training, a balanced subset of 500 clips is sampled, with 200 clips held out for validation. For the diffusion model stages, training utilises all available sequences from the training split, and evaluation is conducted on 487 non-overlapping clips drawn exclusively from the validation sequences.
...
The Fr\'echet Video Distance (FVD)~\cite{unterthiner2019fvd} extends the Fr\'echet Inception Distance to the video domain by comparing distributions of real and generated videos in a learned spatiotemporal feature space. Unlike FID, which evaluates individual frames independently, FVD processes short video clips and therefore captures both visual appearance and temporal dynamics, making it suitable for evaluating generative video models.

Each video is first mapped to a fixed-dimensional feature representation using a pretrained video encoder. A short clip of frames is sampled from the video, resized to the input resolution expected by the encoder, and passed through the network. The feature vector is extracted from the penultimate representation layer after global spatiotemporal pooling, producing a single embedding that summarises the entire clip. Consequently, each video contributes exactly one feature vector to the distribution used for FVD computation.

Following the formulation of FID, the distributions of real and generated video features are each approximated by a multivariate Gaussian. Let $\mathcal{N}(\boldsymbol{\mu}_r, \boldsymbol{\Sigma}_r)$ and $\mathcal{N}(\boldsymbol{\mu}_g, \boldsymbol{\Sigma}_g)$ denote the Gaussian distributions fitted to the feature embeddings of real and generated videos, respectively. The Fr\'echet distance between these two distributions is then given by
\begin{align}
    \text{FVD} = \|\boldsymbol{\mu}_r - \boldsymbol{\mu}_g\|_2^2 + \text{Tr}\!\left(\boldsymbol{\Sigma}_r + \boldsymbol{\Sigma}_g - 2\left(\boldsymbol{\Sigma}_r \boldsymbol{\Sigma}_g\right)^{1/2}\right).
    \label{eq:fvd}
\end{align}
Lower FVD values indicate that the generated videos are closer to the real videos in the learned feature space, reflecting improved visual realism and temporal coherence.

In this work, two different feature backbones are used to compute FVD. The first follows the original formulation based on the Inflated 3D ConvNet (I3D)~\cite{carreira2017quo} pretrained on the Kinetics-400 dataset. I3D employs 3D convolutional filters that jointly model spatial and temporal information and produces a 1024-dimensional video embedding.

The second evaluation uses a content-debiased FVD (CD-FVD)~\cite{ge2024cdfvd} representation based on a pretrained VideoMAE encoder~\cite{tong2022videomae}, a transformer-based model trained using masked video modelling. VideoMAE provides a general spatiotemporal representation that reduces bias towards specific action categories present in the I3D training data and improves sensitivity to motion consistency. In this variant, feature embeddings are extracted from the final pooled representation of the VideoMAE encoder before computing the Fr\'echet distance.

To obtain stable estimates of the Gaussian statistics, FVD is computed over a sufficiently large set of video samples. Following common evaluation practice, approximately 2048 generated videos are used to estimate the feature distributions. Importantly, the sample count refers to the number of video clips rather than individual frames, as each clip contributes a single feature embedding to the distribution.

\subsection{Semantic Consistency Metrics}
\label{subsec:miou}

Semantic consistency is a primary evaluation criterion for both stages of the proposed pipeline. The following metrics are defined here and applied in the evaluation of both stages; the specific evaluation procedures for Stage~1 and Stage~2 differ in how predictions are obtained and are described in Sections~\ref{subsec:stage1} and~\ref{subsec:stage2} respectively.

\paragraph{Mean Intersection-over-Union (mIoU).}
The primary metric for quantifying semantic accuracy is mean Intersection-over-Union (mIoU). For each semantic class $c$, the Intersection-over-Union score is computed from a global confusion matrix accumulated over all evaluation frames:
\begin{align}
    \text{IoU}_c = \frac{\text{TP}_c}{\text{TP}_c + \text{FP}_c + \text{FN}_c},
    \label{eq:iou}
\end{align}
where $\text{TP}_c$, $\text{FP}_c$, and $\text{FN}_c$ denote the true positive, false positive, and false negative pixel counts for class $c$, respectively. The overall mIoU is the macro-average over all classes that appear in the ground truth:
\begin{align}
    \text{mIoU} = \frac{1}{|\mathcal{C}_{\text{valid}}|} \sum_{c \in \mathcal{C}_{\text{valid}}} \text{IoU}_c.
    \label{eq:miou}
\end{align}
Because every class receives equal weight in Equation~\eqref{eq:miou} regardless of its pixel frequency, mIoU penalises failure on rare but semantically important categories such as poles, traffic signs, and pedestrians. This property makes mIoU the preferred metric for assessing holistic semantic comprehension across an imbalanced class distribution.

\paragraph{Frequency-Weighted IoU (FW-IoU).}
Standard mIoU treats rare and frequent classes equally, which can make the metric susceptible to large variance from classes that appear in only a small fraction of frames. The frequency-weighted IoU (FW-IoU) addresses this by weighting each class proportionally to its relative pixel frequency in the ground truth:
\begin{align}
    \text{FW-IoU} = \sum_{c \in \mathcal{C}_{\text{valid}}} f_c \cdot \text{IoU}_c, \qquad f_c = \frac{N_c}{\sum_{c'} N_{c'}},
    \label{eq:fwiou}
\end{align}
where $N_c$ denotes the total number of ground-truth pixels assigned to class $c$ across the evaluation set. FW-IoU reflects the expected IoU for a randomly sampled pixel and is therefore higher than mIoU when the model performs well on spatially dominant categories such as road and vegetation. Reporting both mIoU and FW-IoU provides complementary perspectives: mIoU is informative about tail-class performance, while FW-IoU reflects practical prediction quality across the natural spatial distribution of the scene.

\paragraph{Additional Pixel-Level Metrics.}
Alongside mIoU and FW-IoU, overall pixel accuracy and mean class accuracy are reported. Overall pixel accuracy measures the fraction of correctly classified pixels without regard to class imbalance. Mean class accuracy is the macro-average per-class recall, computed as the fraction of ground-truth pixels of each class that are correctly predicted, then averaged uniformly across all classes present in the ground truth.
...
The proposed pipeline consists of three components:
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{images/semantic_vae_native_u_architecture.pdf}
...
\end{align}
Features are extracted from the decoder core prior to the original RGB output convolution, thereby bypassing the RGB-specific output projection entirely. This is critical: it allows gradient flow through the frozen decoder back to the trainable adapters during backpropagation, while ensuring the RGB output layer is never activated in the semantic pathway.
...
Stage~1 is responsible for predicting a temporally coherent sequence of semantic segmentation maps conditioned on a single RGB frame. This stage adapts the Stable Video Diffusion (SVD) backbone to operate in the latent space of semantic segmentation rather than RGB space. The model learns to generate plausible future scene layouts---road geometry, object positions, and semantic structure---from a single initial observation. Figure~\ref{fig:stage1_pipeline} summarises the Stage~1 processing flow.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\textwidth]{images/stage1_pipeline.pdf}
    \caption{Stage~1 semantic video prediction pipeline. The initial RGB frame provides global scene context through the frozen CLIP encoder, while the ground-truth semantic segmentation maps are encoded by the frozen Semantic VAE into semantic latents. The diffusion backbone predicts denoised semantic latents, which are decoded back into semantic segmentation maps.}
...
\begin{enumerate}
    ...
    \item \textbf{Initial semantic frame latent} $\mathbf{s}^{(0)} \in \mathbb{R}^{B \times 4 \times h \times w}$: The first frame's semantic segmentation map, encoded using the \emph{frozen Semantic VAE}:
    \begin{align}
        \mathbf{s}^{(0)} = \text{SemVAE}_{\text{enc}}(\mathbf{y}^{(0)}),
        \label{eq:initial_frame_latent}
    \end{align}
    where $\mathbf{y}^{(0)} \in \{0, \ldots, C-1\}^{B \times H \times W}$ is the semantic label map of the initial frame. This latent provides spatial structural information---road layout, object positions, and scene geometry---that anchors the predicted semantic sequence. Crucially, the conditioning latent is encoded with the same Semantic VAE used for the target latents, ensuring that both reside in the same latent space.

    \item \textbf{CLIP image embedding} $\mathbf{c}^{(0)} \in \mathbb{R}^{B \times 1 \times d_c}$: The initial \emph{RGB} frame is resized to $224 \times 224$, normalized according to CLIP preprocessing, and passed through a frozen CLIP vision encoder:
    \begin{align}
        \mathbf{c}^{(0)} = \text{CLIP}_{\text{enc}}(\mathbf{f}^{(0)}) \in \mathbb{R}^{B \times 1 \times d_c},
        \label{eq:clip_embedding}
    \end{align}
    where $\mathbf{f}^{(0)} \in \mathbb{R}^{B \times 3 \times H \times W}$ is the initial RGB frame and $d_c = 1024$ is the CLIP embedding dimension. The RGB frame is used \emph{exclusively} for this CLIP encoding step; it is not encoded via the RGB VAE. The CLIP embedding captures high-level semantic context---scene type, weather conditions, lighting, and overall environment---from the initial observation, providing complementary information to the spatial conditioning from the semantic latent.
\end{enumerate}
...
\textit{Channel concatenation conditioning.} The initial semantic frame latent $\mathbf{s}^{(0)}$ is temporally replicated to match the video length and concatenated along the channel dimension with the noisy semantic latents. In the trajectory-conditioned variant, the first and last semantic frame latents are preserved in the conditioning tensor to provide boundary constraints:
\begin{align}
    \mathbf{s}^{(0)}_{\text{pad}} &= \text{Repeat}(\mathbf{s}^{(0)}, T) \in \mathbb{R}^{B \times T \times 4 \times h \times w}, \label{eq:frame_padding} \\
    \mathbf{g} &= \left[\mathbf{s}^{(0)},\, \ldots,\, \mathbf{s}^{(k-1)},\, \mathbf{s}^{(0)},\, \ldots,\, \mathbf{s}^{(0)},\, \mathbf{s}^{(T-1)}\right] \in \mathbb{R}^{B \times T \times 4 \times h \times w}, \label{eq:cond_latents_stage1}
\end{align}
where $k$ denotes the number of initial conditioning semantic frames. The conditioning tensor $\mathbf{g}$ thus contains: (i)~the first $k$ semantic frame latents, (ii)~the initial semantic frame latent repeated for the intermediate frames, and (iii)~the last semantic frame latent for the final position.

A critical design requirement is that all conditioning latents must be encoded using the \emph{same} Semantic VAE as the target latents. Using a different encoder---for example, the RGB VAE for intermediate frames---would introduce incompatible latent distributions. The semantic and RGB latent spaces have fundamentally different statistical properties; the UNet cannot reconcile features drawn from both simultaneously. This constraint keeps the entire channel concatenation pathway within the semantic latent space, providing spatial structural anchoring through the first semantic frame and temporal boundary constraints through the first and last frames. RGB appearance information enters exclusively through the CLIP cross-attention pathway described next.
...
\textit{Cross-attention conditioning.} The CLIP embedding $\mathbf{c}^{(0)}$, derived from the initial RGB frame, is injected via cross-attention layers inside the UNet. This is the sole pathway through which RGB appearance information enters the Stage~1 model. Specifically, the encoder hidden states are repeated across all $T$ frames:
\begin{align}
    \mathbf{c}_{\text{rep}} = \text{RepeatInterleave}(\mathbf{c}^{(0)}, T) \in \mathbb{R}^{(B \cdot T) \times 1 \times d_c},
    \label{eq:clip_repeat}
\end{align}
and used as key and value inputs in the cross-attention layers distributed across the encoder down-sampling blocks and decoder up-sampling blocks of the UNet. This provides global semantic context from the initial frame that guides the generation of all subsequent frames without requiring explicit per-frame conditioning.
...
\end{itemize}
These are projected through sinusoidal embeddings and added element-wise to the diffusion time embedding via a learned linear projection, ensuring that the UNet is aware of the temporal and noise characteristics of the input.
...
The full SVD UNet is finetuned, with the option to restrict gradient updates to only the temporal transformer blocks after an initial warm-up period. This curriculum strategy first allows the model to adapt its full capacity to the semantic domain, then focuses optimization on the temporal layers that govern inter-frame consistency. The Semantic VAE and CLIP encoder remain frozen throughout Stage~1 training. Note that the RGB VAE is not used in Stage~1; all latent encoding (both target and conditioning) is performed exclusively via the Semantic VAE, while the RGB image is only processed by the CLIP encoder for cross-attention conditioning.

\paragraph{Design Rationale.}
Separating semantic prediction from RGB generation reduces learning complexity by allowing the model to focus on scene \emph{structure}---object positions and motion---independently of appearance properties such as texture, lighting, and colour. Operating entirely in semantic space also enforces that generated sequences maintain valid scene layouts, since the output space contains only discrete class identities. Furthermore, because Stage~2 uses ground-truth semantic maps during training, the two stages can be optimized independently and in parallel, and the semantic prediction module can be improved or replaced without retraining the video generation stage. This decomposition mirrors the strategy of Ctrl-V~\cite{luo2025ctrlv}, where high-level structure (bounding box trajectories) is modelled before photorealistic synthesis.

\paragraph{Evaluation of Stage~1.}
Stage~1 is evaluated by comparing the predicted semantic segmentation maps against the corresponding ground-truth label maps. The Semantic VAE decoder converts the predicted latents back to per-pixel class logits, and the class with the highest logit is selected at each pixel via $\arg\max$. The resulting hard predictions are compared against the ground-truth trainID maps using the confusion-matrix-based formulation defined in Section~\ref{subsec:miou}, yielding mIoU, FW-IoU, pixel accuracy, and mean class accuracy over the full validation set. Because Stage~1 outputs are discrete semantic predictions, the evaluation is exact and does not require any external model. During training, mIoU computed on a held-out validation split serves as the model selection criterion. The checkpoint exhibiting the highest validation mIoU is retained as the best model, and training is halted early if no absolute improvement of at least $0.2\%$ in mIoU is observed over six consecutive validation events.

\subsection{Stage~2: Semantic-to-Video Generation}
\label{subsec:stage2}

Stage~2 generates photorealistic RGB video conditioned on semantic segmentation maps. This stage uses a video-adapted ControlNet architecture~\cite{zhang2023adding} that injects semantic conditioning into the frozen SVD backbone, learning to translate semantic layouts into realistic driving scenes while preserving temporal coherence. Figure~\ref{fig:stage2_pipeline} illustrates the Stage~2 generation process.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\textwidth]{images/stage2_pipeline.pdf}
    \caption{Stage~2 semantic-to-video generation pipeline. Ground-truth semantic segmentation maps are encoded into semantic latents that condition the ControlNet branch, while the initial RGB frame is encoded by the frozen CLIP encoder and RGB VAE to provide appearance and frame conditioning. The frozen SVD backbone and ControlNet jointly predict RGB video latents, which are decoded into the final photorealistic video sequence.}
...
Freezing the SVD UNet during Stage~2 training serves multiple purposes. First, the backbone retains its learned knowledge of natural video dynamics, appearance, and temporal coherence; updating its parameters would risk catastrophic forgetting and degrade the pretrained generation quality. Second, training only the ControlNet parameters substantially reduces GPU memory requirements and accelerates convergence. Third, the zero-initialized residual injection architecture (Section~\ref{subsec:conditioning_mechanisms}) ensures that the ControlNet learns to \emph{steer} the frozen backbone's existing capabilities rather than override them, providing a stable optimization trajectory from the outset.
...
The decomposed architecture provides explicit, pixel-level scene control through semantic maps while maintaining modularity: each stage can be trained, evaluated, and improved independently. Ground-truth semantic conditioning during Stage~2 training enables the two stages to be optimized in parallel, reducing the overall training time. Temporal attention mechanisms in both stages enforce coherent frame-to-frame transitions. Finally, the use of frozen backbones with lightweight adapters---finetuned temporal layers in Stage~1 and a ControlNet in Stage~2---enables effective training with limited computational resources.

\paragraph{Evaluation of Stage~2.}
Stage~2 outputs RGB video frames rather than semantic segmentation maps, so a direct pixel-level comparison against ground-truth semantic labels is not applicable. Instead, a pretrained semantic segmentation network is applied to each generated RGB frame to infer its semantic content, and the resulting predictions are compared against the corresponding ground-truth labels. Specifically, a Dilated Residual Network (DRN)~\cite{yu2017dilated} — pre-trained on the KITTI-360 training sequences — serves as the external evaluator. Applying this fixed network to the generated frames and accumulating a confusion matrix against the ground-truth labels yields DRN mIoU, DRN pixel accuracy, DRN mean class accuracy, and DRN FW-IoU for each generated clip, using the metric formulations defined in Section~\ref{subsec:miou}.

This indirect evaluation provides a principled measure of semantic controllability. If the generated RGB frames faithfully preserve the scene layout specified by the conditioning maps, a competent segmentation model applied to those frames should recover predictions that closely match the original ground-truth labels. The DRN mIoU therefore quantifies the extent to which the generation process respects the semantic structure of the conditioning input. It is important to note that DRN mIoU is not a direct measure of photometric generation quality; it reflects semantic consistency as assessed by an external model whose own accuracy on real images forms an implicit performance ceiling.

During Stage~2 training, the early stopping criterion is the Learned Perceptual Image Patch Similarity (LPIPS)~\cite{zhang2018unreasonable} evaluated on held-out validation clips, rather than DRN mIoU. This choice reflects the nature of Stage~2 as an RGB video generation task: LPIPS captures both structural similarity and perceptual appearance using deep feature representations and therefore provides a more sensitive and stable training signal than the computationally expensive DRN-based evaluation. The checkpoint yielding the lowest validation LPIPS is retained as the best model, and training terminates if no absolute reduction of at least $0.005$ in LPIPS is observed over eight consecutive validation events.
