...
\end{align}
where $k$ denotes the number of initial conditioning semantic frames. The conditioning tensor $\mathbf{g}$ thus contains: (i)~the first $k$ semantic frame latents, (ii)~the initial semantic frame latent repeated for the intermediate frames, and (iii)~the last semantic frame latent. This design keeps the entire channel concatenation pathway within the semantic latent space, providing spatial structural anchoring (via the first semantic frame) and temporal boundary constraints (via the first and last semantic frames). The RGB appearance information enters exclusively through the CLIP cross-attention pathway.
