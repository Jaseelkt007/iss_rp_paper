...
A closer reference to the present thesis is SVS-GAN~\cite{seyam2024svsgan}, which formulates semantic video synthesis as the generation of RGB video from a reference image and a sequence of semantic segmentation maps. Unlike latent diffusion approaches, SVS-GAN adopts an autoregressive GAN architecture with semantic-specific losses and reports strong results on Cityscapes and KITTI-360. Its importance for the present work lies in problem alignment rather than architectural inheritance: it demonstrates that dense semantic maps are a viable control signal for driving video generation, but does so outside the pretrained SVD-ControlNet paradigm used here.

These works collectively demonstrate the feasibility of generative driving simulation, but they occupy different points on the controllability spectrum. World-model and layout-conditioned methods rely on high-level or sparse structure, Ctrl-V focuses on object-centric bounding-box control, and SVS-GAN targets dense semantic-map-conditioned synthesis in an autoregressive GAN setting. The present thesis addresses the intersection of dense semantic control and pretrained latent video diffusion: a modular ControlNet-style framework for autonomous driving that uses semantic segmentation maps as the primary conditioning signal.
...
The primary distinction from prior work lies in the conditioning modality and its implications for scene representation. Sparse bounding box conditioning constrains object locations but not background structure; dense semantic conditioning constrains every pixel, providing complete scene-level specification at every frame. The primary contribution of this thesis is therefore not the two-stage decomposition itself, which follows Ctrl-V, but the study of dense semantic control within a pretrained latent diffusion pipeline for autonomous driving. More specifically, the thesis combines dense semantic conditioning, a semantic-native latent interface, and modular ControlNet-based video synthesis in a setting where prior work has typically explored either sparse diffusion-based control or dense semantic synthesis in non-diffusion autoregressive frameworks. Chapter~\ref{ch:methodology} describes the proposed methodology in detail.
