...
It is instructive to compare the Stage~1 semantic prediction metrics with the Stage~2 semantic fidelity metrics (Table~\ref{tab:semantic_fidelity}). Stage~1 achieves a mIoU of 40.01\% in direct semantic prediction (evaluated on 170 clips), while Stage~2 achieves 45.60\% mIoU as measured by re-segmenting the generated RGB frames with DRN-D-105 (evaluated on 150 clips with ground-truth semantic conditioning). This comparison must be interpreted carefully: Stage~2 uses \emph{ground-truth} semantic maps as conditioning during this evaluation, which represents an upper bound on Stage~2 performance in the full end-to-end pipeline. Nevertheless, the result suggests that the primary performance bottleneck in end-to-end inference lies in predicting accurate future semantic layouts (Stage~1) rather than translating those layouts into photorealistic appearance (Stage~2). Improvements to Stage~1 prediction quality would therefore directly benefit end-to-end generation fidelity.
