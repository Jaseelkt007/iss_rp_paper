...
The Stage~1 model (checkpoint at training step 10{,}200) generates 25-frame semantic video sequences at $192 \times 704$ resolution using 30 denoising steps. The generated semantic latents are decoded via the frozen Semantic VAE to produce per-pixel class predictions, which are compared against ground-truth semantic labels using standard segmentation metrics. The evaluation is conducted on 170 validation clips (4{,}250 frames total), providing a comprehensive assessment across diverse driving scenarios in the KITTI-360 dataset.
...
The mIoU of 40.01\% indicates that the diffusion model captures the broad semantic structure of driving scenes---road layout, building placement, vegetation distribution---while struggling with fine-grained and rare classes. The pixel accuracy of 80.84\% and frequency-weighted IoU of 68.75\% confirm that spatially dominant classes are predicted reliably. The gap between pixel accuracy and mIoU reflects the well-known sensitivity of mIoU to rare classes, which contribute equally to the metric despite occupying a small fraction of image pixels. Results are reported on 170 validation clips, providing robust statistics across diverse suburban driving scenarios.
...
The results reveal a clear tiered structure tied to class frequency and spatial extent:

\begin{itemize}
    \item \textbf{Well-predicted classes (IoU $>$ 60\%):} Road (84.28\%), building (74.08\%), sky (70.75\%), vegetation (69.35\%), truck (64.10\%), sidewalk (57.84\%), and car (56.71\%). These spatially dominant classes with large contiguous regions are predicted reliably, demonstrating that the model successfully learns the overall scene layout from the initial RGB frame. Truck achieves notably high IoU (64.10\%) despite being a dynamic object class, suggesting the model captures vehicle size and positioning well.

    \item \textbf{Moderately predicted classes (IoU 15--60\%):} Terrain (54.45\%), fence (45.11\%), wall (37.75\%), bus (24.54\%), traffic sign (19.72\%), person (18.54\%), and motorcycle (15.46\%). These classes exhibit moderate performance due to either lower spatial frequency, irregular shapes, or fewer training examples. Bus achieves 24.54\% IoU, indicating partial success in capturing large vehicle structures in the semantic sequence.

    \item \textbf{Poorly predicted classes (IoU $<$ 15\%):} Pole (15.02\%), rider (12.40\%), bicycle (0.00\%), and traffic light (0.00\%). These rare, small-scale classes are extremely challenging to predict in future frames due to their limited pixel coverage and high spatial variability. The model fails entirely on bicycle and traffic light, which have few ground-truth pixels across the evaluation set.
\end{itemize}

\subsection{Variation Analysis}
\label{subsec:stage1_per_sample}

Across the 170 validation clips, per-clip mIoU varies considerably, reflecting the diversity of scene complexity in the KITTI-360 dataset. Clips featuring relatively static, well-structured suburban scenes achieve higher semantic prediction quality, while clips with complex dynamics, unusual viewpoints, or rare object classes exhibit lower accuracy. This variation is expected for a generative diffusion model, which must predict plausible future semantic layouts rather than perform deterministic reconstruction.

\subsection{Comparison with Stage~2 Semantic Fidelity}
\label{subsec:stage1_vs_stage2}

It is instructive to compare the Stage~1 semantic prediction metrics with the Stage~2 semantic fidelity metrics (Table~\ref{tab:semantic_fidelity}). Stage~1 achieves a mIoU of 40.01\% in direct semantic prediction (evaluated on 170 clips), while Stage~2 achieves 39.17\% mIoU as measured by re-segmenting the generated RGB frames with DRN-D-105 (evaluated on 487 clips with ground-truth semantic conditioning). This comparison must be interpreted carefully: Stage~2 uses \emph{ground-truth} semantic maps as conditioning during this evaluation, which represents an upper bound on Stage~2 performance in the full end-to-end pipeline. The two metrics are close in magnitude, suggesting that both stages contribute comparably to the end-to-end semantic fidelity. Improvements to Stage~1 prediction quality and to Stage~2 controllability would each contribute to end-to-end generation performance.
...
\begin{itemize}
    \item \textbf{Fr\'{e}chet Inception Distance (FID)}~\cite{heusel2017gans}: Measures distributional similarity between generated and real frames using Inception-v3 features.
...
Table~\ref{tab:video_metrics} summarizes the image and video quality metrics for the Stage~2 model with UNet partial unfreezing (step~32,700, 487 validation clips).

\begin{table}[t]
    \centering
    \caption{Stage~2 image and video quality metrics on KITTI-360 validation set (step~32,700, 487 clips)}
...
\paragraph{FID.}
The FID of 21.91 places the model in a competitive range for driving video generation (typical values for comparable methods: 15--40), confirming that the generated frames are perceptually realistic at the distribution level. The frozen SVD backbone preserves its learned natural image prior while the partially unfrozen decoder adapts to semantic conditioning.

\paragraph{FVD.}
The FVD-I3D of 255.21 indicates moderate temporal coherence. The metric captures both per-frame appearance quality and inter-frame consistency via I3D video features. The proposed multi-scale conditioning extension is expected to reduce this value by providing stronger semantic grounding at each UNet resolution scale, reducing frame-to-frame semantic layout drift.

\paragraph{Perceptual and Pixel-Level Metrics.}
The LPIPS of 0.357 indicates perceptual similarity to ground-truth frames, with expected differences in fine-grained details such as textures and lighting. The moderate SSIM (0.443) and PSNR (14.62~dB) are consistent with generative models that produce \emph{plausible} outputs rather than pixel-identical reconstructions; distributional measures (FID, FVD) are therefore more informative for evaluating generation quality.

\subsection{Semantic Fidelity}
\label{subsec:semantic_fidelity}

To evaluate whether the generated RGB frames faithfully encode the semantic structure specified by the conditioning maps, we apply a pretrained DRN-D-105 segmentation network~\cite{yu2017dilated} (trained on KITTI-360 with 19 classes) to segment the generated RGB frames and compare the predicted segmentation against the ground-truth labels. This evaluation is conducted on the full set of 487 validation clips (25 frames each), providing a statistically robust assessment across diverse driving scenarios.

\paragraph{Overall Metrics.}
Table~\ref{tab:semantic_fidelity} presents the overall semantic fidelity metrics for the UNet partial unfreezing configuration.

\begin{table}[t]
    \centering
    \caption{Semantic fidelity of Stage~2 generated RGB frames (step~32,700, 487 clips), measured by applying DRN-D-105 segmentation to generated frames and comparing against ground-truth labels}
...
The mIoU of 39.17\% indicates that generated RGB frames contain sufficient visual structure for an external segmentation network to recover the intended semantic layout for the majority of classes. The pixel accuracy of 85.34\% and frequency-weighted IoU of 75.31\% confirm that spatially dominant classes are generated with high fidelity. The gap between pixel accuracy and mIoU reflects the persistent difficulty of generating rare or small-scale classes, which contribute equally to mIoU despite their limited spatial extent.

\paragraph{Per-Class Analysis.}
Table~\ref{tab:stage2_per_class} presents the per-class IoU across all 19 semantic categories for the UNet partial unfreezing configuration.

\begin{table}[t]
    \centering
    \caption{Per-class IoU for Stage~2 semantic fidelity evaluation (step~32,700, 487 clips). Classes grouped by generation quality tier}
...
The results reveal a clear tiered structure governed by class frequency and spatial extent:

\begin{itemize}
    \item \textbf{Well-generated classes (IoU $>$ 70\%):} Road (85.17\%), sky (84.37\%), car (84.29\%), vegetation (80.62\%), and building (75.25\%). These spatially dominant classes with large contiguous regions are generated with high fidelity, confirming that the ControlNet conditioning effectively translates large-scale semantic structure into photorealistic appearance.

    \item \textbf{Moderately generated classes (IoU 20--70\%):} Terrain (59.06\%) and sidewalk (55.20\%) benefit from large surface coverage, while wall (47.63\%) and fence (37.74\%) rely on boundary-precise generation. Traffic sign (30.99\%) and pole (25.67\%) represent thin or elevated structures that are challenging to render at the operating resolution.

    \item \textbf{Poorly generated classes (IoU $<$ 20\%):} Rider (9.88\%), bus (7.34\%), motorcycle (5.17\%), bicycle (3.15\%), and train (0.32\%) remain constrained by class rarity and limited spatial extent in the 24$\times$88 latent space. Traffic light (0.00\%) occupies fewer than 341 ground-truth pixels across the entire evaluation set, placing it effectively below the latent-space resolution threshold.
\end{itemize}
...
To address this, we selectively unfroze a minimal, high-impact subset of the UNet decoder: the bottleneck mid-block---where the primary ControlNet residuals are injected at the lowest spatial resolution---together with the final normalisation and output projection layers that project the decoder's feature channels back to the 4-channel latent space. This subset corresponds to approximately 15--20\% of total UNet parameters. The UNet encoder and all decoder up-sampling blocks remained frozen to preserve SVD's spatial and temporal feature extraction priors and to stay within GPU memory constraints (48~GB VRAM). The unfrozen parameters were trained with a lower learning rate ($5 \times 10^{-6}$) than the ControlNet to prevent catastrophic forgetting. The semantic VAE latent scaling factor was applied in this configuration to bring the conditioning and noisy RGB latents into compatible magnitude ranges.

Table~\ref{tab:unet_unfreeze_comparison} presents the improvement in semantic fidelity resulting from this intervention.

\begin{table}[t]
    \centering
    \caption{Stage~2 semantic fidelity comparison: frozen UNet baseline vs.\ UNet partial unfreezing (DRN-D-105, KITTI-360 validation set)}
...
The intervention yielded a gain of $+15.97$ percentage points in mIoU (23.20\% $\rightarrow$ 39.17\%), the single largest improvement observed across all training interventions in this work. Dominant classes benefited substantially: car improved from 40.61\% to 84.29\% IoU, sky from 55.11\% to 84.37\%, and road from 70.66\% to 85.17\%. Notably, the unfreeze model reached this performance at step~32,700---fewer than half the steps required by the frozen baseline (96,100)---confirming that the architectural change had an immediate and high-magnitude effect on gradient signal quality. Small and rare classes (traffic light, bicycle, train) remain near-zero, a limitation attributed to the 24$\times$88 effective latent resolution rather than training capacity. These residual gaps motivate the multi-scale conditioning extension described in Section~\ref{sec:future_work}.
...
\begin{itemize}
    ...
    \item \textbf{Object placement:} Vehicles, pedestrians, and other foreground objects appear at locations consistent with the conditioning semantic map. The car class is particularly well-reproduced, aligning with its high per-class IoU of 84.29\%.
